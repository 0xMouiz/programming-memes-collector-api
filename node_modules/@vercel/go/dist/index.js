...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.createGo = exports.GoWrapper = exports.getAnalyzedEntrypoint = exports.OUT_EXTENSION = exports.goGlobalCachePath = exports.localCacheDir = void 0;
...
exports.localCacheDir = path_1.join('.vercel', 'cache', 'golang');
 ...
 */
async function getAnalyzedEntrypoint({ entrypoint, modulePath, workPath, }) {
    ...
    try {
        build_utils_1.debug(`Analyzing entrypoint ${entrypoint} with modulePath ${modulePath}`);
        const args = [`-modpath=${modulePath}`, path_1.join(workPath, entrypoint)];
        analyzed = await execa_1.default.stdout(bin, args);
    }
    catch (err) {
...
class GoWrapper {
    ...
    execute(...args) {
        const { opts, env } = this;
        build_utils_1.debug(`Exec: go ${args.map(a => (a.includes(' ') ? `"${a}"` : a)).join(' ')}`);
        build_utils_1.debug(`  CWD=${opts.cwd}`);
        build_utils_1.debug(`  GOROOT=${(env || opts.env).GOROOT}`);
        build_utils_1.debug(`  GO_BUILD_FLAGS=${(env || opts.env).GO_BUILD_FLAGS}`);
    ...
    build(src, dest) {
        ...
        const flags = envGoBuildFlags ? string_argv_1.default(envGoBuildFlags) : GO_FLAGS;
...
async function createGo({ modulePath, opts = {}, workPath, }) {
    ...
    const goCacheDir = path_1.join(workPath, exports.localCacheDir);
    ...
    for (const [label, goDir] of Object.entries(goDirs)) {
        try {
            ...
            if (version === goSelectedVersion || short === goSelectedVersion) {
                build_utils_1.debug(`Selected go ${version} (from ${label})`);
...
async function parseGoModVersion(modulePath) {
    ...
    try {
        ...
        if (major === 1 && minor >= GO_MIN_VERSION && full) {
            ...
            const err = new GoError(`Unsupported Go version ${major}.${minor} in ${file}`);
            err.code = 'ERR_UNSUPPORTED_GO_VERSION';
            throw err;
        }
        else {
...
const HANDLER_FILENAME = `handler${go_helpers_1.OUT_EXTENSION}`;
...
async function build({ files, entrypoint, config, workPath, meta = {}, }) {
  ...
  `);
        ...
        if (!goModPath && (await fs_extra_1.pathExists(path_1.join(workPath, 'vendor')))) {
            throw new Error('`go.mod` is required to use a `vendor` directory.');
        }
        const analyzed = await go_helpers_1.getAnalyzedEntrypoint({
            entrypoint,
            modulePath: goModPath ? path_1.dirname(goModPath) : undefined,
            workPath,
        });
        ...
        if (config && config.includeFiles) {
            ...
            for (const pattern of patterns) {
                const fsFiles = await build_utils_1.glob(pattern, entrypointDirname);
                for (const [assetName, asset] of Object.entries(fsFiles)) {
                    includedFiles[assetName] = asset;
                }
            }
        }
        const modulePath = goModPath ? path_1.dirname(goModPath) : undefined;
 ...
 */
async function buildHandlerWithGoMod({ downloadPath, entrypoint, entrypointAbsolute, entrypointDirname, go, goModPath, handlerFunctionName, isGoModInRootDir, outDir, packageName, undo, }) {
    build_utils_1.debug(`Building Go handler as package "${packageName}" (with${goModPath ? '' : 'out'} go.mod)`);
    let goModDirname;
    if (goModPath !== undefined) {
        goModDirname = path_1.dirname(goModPath);
        // first we backup the original
        const backupFile = path_1.join(goModDirname, `__vc_go.mod.bak`);
        await fs_extra_1.copy(goModPath, backupFile);
        undo.fileActions.push({
            to: goModPath,
            from: backupFile,
        });
        const goSumPath = path_1.join(goModDirname, 'go.sum');
        const isGoSumExists = await fs_extra_1.pathExists(goSumPath);
        if (!isGoSumExists) {
    ...
    const relPackagePath = goModDirname
        ? path_1.posix.relative(goModDirname, entrypointDirname)
        : '';
    if (relPackagePath) {
        goPackageName = path_1.posix.join(packageName, relPackagePath);
    }
    let mainGoFile;
    if (goModPath && isGoModInRootDir) {
        build_utils_1.debug(`[mod-root] Write main file to ${downloadPath}`);
        mainGoFile = path_1.join(downloadPath, MAIN_GO_FILENAME);
    }
    else if (goModDirname && !isGoModInRootDir) {
        build_utils_1.debug(`[mod-other] Write main file to ${goModDirname}`);
        mainGoFile = path_1.join(goModDirname, MAIN_GO_FILENAME);
    }
    else {
        build_utils_1.debug(`[entrypoint] Write main file to ${entrypointDirname}`);
        mainGoFile = path_1.join(entrypointDirname, MAIN_GO_FILENAME);
    }
    await Promise.all([
    ...
    try {
        ...
        if (!goModPath || path_1.dirname(entrypointAbsolute) === path_1.dirname(goModPath)) {
            ...
            undo.directoryCreation.push(path_1.dirname(finalDestination));
        }
    }
    catch (err) {
    ...
    if (goModPath && isGoModInRootDir) {
        ...
        baseGoModPath = path_1.dirname(goModPath);
    }
    else {
    ...
    const destPath = path_1.join(outDir, HANDLER_FILENAME);
    try {
        const src = [path_1.join(baseGoModPath, MAIN_GO_FILENAME)];
 ...
 */
async function buildHandlerAsPackageMain({ entrypointAbsolute, entrypointDirname, go, handlerFunctionName, outDir, undo, }) {
    build_utils_1.debug('Building Go handler as package "main" (legacy)');
    await writeEntrypoint(path_1.join(entrypointDirname, MAIN_GO_FILENAME), '', handlerFunctionName);
    undo.fileActions.push({
        to: undefined,
        from: path_1.join(entrypointDirname, MAIN_GO_FILENAME),
    });
    ...
    try {
        const src = [
            path_1.join(entrypointDirname, MAIN_GO_FILENAME),
            entrypointAbsolute,
 ...
 */
async function cleanupFileSystem({ fileActions, directoryCreation, functionRenames, }) {
    ...
    for (const action of fileActions.reverse()) {
        if (action.to) {
            await fs_extra_1.move(action.from, action.to, { overwrite: true });
        }
        else {
...
async function findGoModPath(entrypointDir, workPath) {
    let goModPath = undefined;
    let isGoModInRootDir = false;
    let dir = entrypointDir;
    while (!isGoModInRootDir) {
        isGoModInRootDir = dir === workPath;
        const goMod = path_1.join(dir, 'go.mod');
        if (await fs_extra_1.pathExists(goMod)) {
            goModPath = goMod;
            build_utils_1.debug(`Found ${goModPath}"`);
            break;
        }
...
async function writeEntrypoint(dest, goPackageName, goFuncName) {
    ...
    await fs_extra_1.writeFile(dest, mainModGoContents, 'utf-8');
}
/**
 ...
 */
async function writeGoMod({ destDir, goModPath, packageName, }) {
    let contents = `module ${packageName}`;
    if (goModPath) {
        const goModRelPath = path_1.relative(destDir, path_1.dirname(goModPath));
        const goModContents = await fs_extra_1.readFile(goModPath, 'utf-8');
        contents = goModContents
            .replace(/^module\s+.+$/m, contents)
            .replace(/^(replace .+=>\s*)(.+)$/gm, (orig, replaceStmt, replacePath) => {
            if (replacePath.startsWith('.')) {
                return replaceStmt + path_1.join(goModRelPath, replacePath);
            }
            return orig;
        });
        ...
        if (moduleName) {
            let relPath = path_1.normalize(goModRelPath);
            if (!relPath.endsWith('/')) {
                relPath += '/';
            }
            const requireRE = new RegExp(`require\\s+${moduleName}`);
            const requireGroupRE = new RegExp(`require\\s*\\(.*${moduleName}.*\\)`, 's');
    ...
    const destGoModPath = path_1.join(destDir, 'go.mod');
    build_utils_1.debug(`Writing ${destGoModPath}`);
    // console.log(contents);
    await fs_extra_1.writeFile(destGoModPath, contents, 'utf-8');
}
/**
...
async function findGoWorkFile(goWorkDir, workPath) {
    while (!(await fs_extra_1.pathExists(path_1.join(goWorkDir, 'go.work')))) {
        if (goWorkDir === workPath) {
            return;
        }
        goWorkDir = path_1.dirname(goWorkDir);
    }
    return path_1.join(goWorkDir, 'go.work');
}
/**
...
async function writeGoWork(destDir, workPath, modulePath) {
    ...
    if (goWorkPath) {
        const contents = await fs_extra_1.readFile(goWorkPath, 'utf-8');
        const addPath = (path) => {
            if (path) {
                if (path.startsWith('.')) {
                    workspaces.add(path_1.relative(destDir, path_1.join(workPath, path)));
                }
                else {
                    workspaces.add(path);
                }
        ...
        workspaces.add(path_1.relative(destDir, modulePath));
    }
    const contents = `use (\n${Array.from(workspaces)
        .map(w => `  ${w}\n`)
        .join('')})\n`;
    // console.log(contents);
    await fs_extra_1.writeFile(path_1.join(destDir, 'go.work'), contents, 'utf-8');
}
async function startDevServer(opts) {
    ...
    const { goModPath } = await findGoModPath(path_1.join(workPath, entrypointDir), workPath);
    const modulePath = goModPath ? path_1.dirname(goModPath) : undefined;
    const analyzed = await go_helpers_1.getAnalyzedEntrypoint({
...
async function prepareCache({ workPath, }) {
    ...
    const goCacheDir = path_1.join(workPath, go_helpers_1.localCacheDir);
    ...
    const cache = await build_utils_1.glob(`${go_helpers_1.localCacheDir}/**`, workPath);
    return cache;
}
